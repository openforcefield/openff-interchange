...
def to_prmtop(interchange: "Interchange", file_path: Path | str):
    ...
    with open(path, "w") as prmtop:
        ...
        per_atom_exclusion_lists = _get_per_atom_exclusion_lists(interchange._topology)

        number_excluded_atoms, excluded_atoms_list = _get_exclusion_lists(
            per_atom_exclusion_lists,
        )
        atomic_numbers = tuple(atom.atomic_number for atom in interchange._topology.atoms)
        ...
        # number of residues
        NRES = max(1, len([*interchange._topology.hierarchy_iterator("residues")]))
        ...
        NMXRS = interchange._topology.n_atoms
        ...
        for atom in interchange._topology.atoms:
        ...
        masses = [a.mass.m for a in interchange._topology.atoms]
        ...
        residue_names = [
            getattr(residue, "residue_name", "RES") for residue in interchange._topology.hierarchy_iterator("residues")
        ]
        ...
        residue_pointers = (
            [
                interchange._topology.atom_index(next(iter(residue.atoms)))
                for residue in interchange._topology.hierarchy_iterator("residues")
            ]
            if NRES > 1
            else [0]
        )
        ...
        if IFBOX == 1:
            ...
            prmtop.write(str(interchange._topology.n_atoms).rjust(8))
            prmtop.write("\n")
...
def to_inpcrd(interchange: "Interchange", file_path: Path | str):
    ...
    n_atoms = interchange._topology.n_atoms
    time = 0.0
