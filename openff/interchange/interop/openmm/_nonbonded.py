...
def _add_particles_to_system(
    ...
    for molecule in topology.molecules:
        for virtual_site_key in molecule_virtual_site_map[topology.molecule_index(molecule)]:
...
def _create_single_nonbonded_force(
    ...
    if interchange.box is None:
        if (data.vdw_method in ("no-cutoff", None)) and (data.electrostatics_method in ("Coulomb", None)):
        ...
        for atom in molecule.atoms:
            ...
            if data.electrostatics_collection is not None:
                if has_virtual_sites:
                    ...
                    partial_charge = atomic_partial_charges[atom_index]
    ...
    for molecule in topology.molecules:
        ...
        for virtual_site_key in molecule_virtual_site_map[molecule_index]:
            ...
            virtual_site_potential = interchange["VirtualSites"].potentials[_potential_key]
            virtual_site_object = _create_virtual_site_object(
                virtual_site_key,
                virtual_site_potential,
            )
...
def _create_multiple_nonbonded_forces(
    ...
    for atom1, atom2 in _get_14_pairs(topology):
        openff_indices = (
            topology.atom_index(atom1),
            topology.atom_index(atom2),
        )
    ...
    for molecule in interchange.get_topology().molecules:
        if has_virtual_sites:
            ...
            for virtual_site_key in molecule_virtual_site_map[molecule_index]:
                ...
                parent_virtual_particle_mapping[openff_openmm_particle_map[parent_atom_index]].append(force_index)
