...
class TestLammps:
    ...
    def test_to_lammps_single_mols(self, mol : str, sage_unconstrained : ForceField, n_mols : int) -> None:
        """
    ...
    def test_unique_lammps_mol_ids(self, smiles : str, sage_unconstrained : ForceField, sidelen : int=0) -> bool:
        '''Test to see if interop.lammps.export._write_atoms() writes unique ids for each distinct Molecule'''
        temp_lammps_path = Path.cwd() / 'temp.lmp' # NOTE: lammps writer doesn't currently support IO-like object passing, so need to intercept output with temporary file
        ...
        xyz_offsets = numpy.column_stack([ # integral offsets from 0...(sidelen - 1) along 3 axes
            axis_offsets.ravel() for axis_offsets in numpy.meshgrid(
                *[numpy.arange(sidelen) for _ in range(3)] # the 3 here is for 3-dimensions
            )
        ])
        ...
        for int_offset in xyz_offsets:
            ...
            mol.add_conformer((conf_centered + 2*r_eff*int_offset).to('nm')) # space copied by effective diameter
        ...
        box_vectors = numpy.eye(3) * box_dims # convert to diagonal matrix to get proper shape

        # EXPORT TO LAMMPS
        interchange = Interchange.from_smirnoff(sage_unconstrained, tiled_top)#, charge_from_molecules=[pilot_mol])
        ...
        with temp_lammps_path.open('r') as lmp_file: # pull out text from temporary lammps file ...
            all_lines = [line for line in lmp_file.read().split('\n') if line]          # ... separate by newlines, remove empty lines ...
            atom_lines = all_lines[all_lines.index('Atoms')+1:all_lines.index('Bonds')] #... extract atoms block ...
        ...
        def extract_info_from_lammps_atom_lines(atom_line : str) -> Dict[str, Any]:
            ...
            return {
                field_label : FieldType(str_val)
                    for str_val, (field_label, FieldType) in zip(atom_line.split('\t'), KEYWORDS.items())
            }
        ...
        expected_mol_ids = set(i + 1 for i in range(interchange.topology.n_molecules)) # we'd like for each of the N molecules to have ids from [1...N]
