...
def to_lammps(openff_sys: System, file_path: Union[Path, str]):
    ...
    n_atoms = openff_sys.topology.n_topology_atoms
    ...
    with open(path, "w") as lmp_file:
        ...
        _write_pair_coeffs(lmp_file=lmp_file, openff_sys=openff_sys)
        ...
        _write_atoms(lmp_file=lmp_file, openff_sys=openff_sys)


def _write_pair_coeffs(lmp_file: IO, openff_sys: System):
...
def _write_atoms(lmp_file: IO, openff_sys: System):
    lmp_file.write("Atoms\n\n")

    molecule_map = dict(enumerate(openff_sys.topology.topology_molecules))
    molecule_map_inv = dict({v: k for k, v in molecule_map.items()})

    electrostatics_handler = openff_sys.handlers["Electrostatics"]
    for atom_idx, atom in enumerate(openff_sys.topology.n_topology_atoms):
